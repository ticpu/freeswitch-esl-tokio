...
| `commands` | `Originate`, `BridgeDialString`, `FlattenedDialString` (a switch-produced dial string read as the switch reads it), `UuidKill`, `UuidBridge`, endpoint types, `BlockParse` *(requires `esl` feature)* |
