...
impl EslParser {
    ...
    /// Whether the parser is waiting for headers (no partial body state).
    pub(crate) fn is_waiting_for_headers(&self) -> bool {
        matches!(self.state, ParseState::WaitingForHeaders)
    }

    /// Return a view of the unconsumed data in the parser buffer.
    ...
    pub(crate) fn parse_headers(&self, headers_str: &str) -> EslResult<HashMap<String, String>> {
    ...
    fn parse_xml_event(&self, message: EslMessage) -> EslResult<EslEvent> {
        ...
        // quick-xml 0.39 splits text around entity references (e.g.
        // "Smith &amp; Jones" becomes Text, GeneralRef, Text), so we
        // must accumulate fragments and flush on End tags.
    ...
    fn resolve_entity(entity: &quick_xml::events::BytesRef<'_>) -> EslResult<String> {
        ...
        let name = std::str::from_utf8(entity.as_ref())?;
        match name {
            "amp" => Ok("&".to_string()),
            "lt" => Ok("<".to_string()),
            "gt" => Ok(">".to_string()),
            "quot" => Ok("\"".to_string()),
            "apos" => Ok("'".to_string()),
            _ => Err(EslError::protocol_error(format!(
